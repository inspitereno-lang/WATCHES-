          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-2 lg:hidden">
                ...
                <div className="relative aspect-[4/5] sm:aspect-[3/4] overflow-hidden bg-[#111] border-r border-white/10">
                  <img
                    src={match.image}
                  ...
                  {/* Item Number Badge */}
                  <div className="absolute top-2 left-2 font-mono text-[8px] font-bold text-[#e8c264] bg-black/75 border border-[#e8c264]/30 px-1.5 py-0.5 rounded">
                    {itemNumber}
                  </div>

                  {/* Exact Reference Indicator */}
                  <div className="absolute top-2 right-2 flex items-center gap-1 rounded-full border border-emerald-300/30 bg-black/75 px-1.5 py-0.5 text-[7px] font-mono text-emerald-300 backdrop-blur-md">
                  ...
                  <div className="absolute bottom-2.5 left-2.5 right-2.5 text-left">
                    <p className="font-mono text-[7px] uppercase tracking-[0.2em] text-[#e8c264]">
                      {translate("Seen on", currentLang)}
                    </p>
                    <h3 className="font-display text-sm leading-tight text-white font-semibold line-clamp-1">
                      {translate(match.celebrity, currentLang)}
                    </h3>
                  </div>
                </div>

                {/* Mobile Tile 2: Exact Matching Watch Card */}
                <div className="relative flex flex-col justify-between bg-[radial-gradient(circle_at_50%_40%,rgba(212,175,55,0.14),transparent_65%),linear-gradient(145deg,#120e09_0%,#080807_75%)] p-2.5 sm:p-4">
                  {/* Top Row: Brand & In Stock */}
                  <div className="flex items-center justify-between border-b border-white/10 pb-1.5">
                    <span className="font-mono text-[8px] font-semibold uppercase tracking-wider text-[#e8c264] truncate">
                  ...
                  {/* Watch Image with subtle circular aura */}
                  <div className="relative flex items-center justify-center py-2 flex-1 my-auto">
                    <div className="absolute h-20 w-20 sm:h-24 sm:w-24 rounded-full border border-[#e8c264]/15" />
                    {match.watchImage || product?.image ? (
                      <WatchImage
                        src={match.watchImage || product?.image || ''}
                        alt={match.reference}
                        loading={index === 0 ? 'eager' : 'lazy'}
                        className="icon-watch relative z-10 max-h-20 sm:max-h-24 max-w-[85%] object-contain drop-shadow-[0_8px_18px_rgba(0,0,0,0.8)]"
                      />
                    ) : (
                      <div className="h-16 w-16 animate-pulse rounded-full border border-[#e8c264]/10 bg-[#e8c264]/5" />
                    )}
                  </div>

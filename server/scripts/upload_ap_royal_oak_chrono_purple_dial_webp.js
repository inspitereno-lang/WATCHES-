...
const TEMP_DIR = path.join(__dirname, 'temp_webp_ro_chrono_purple');
...
async function main() {
  try {
    console.log('========================================================================');
    console.log('Processing Royal Oak Chronograph Selfwinding Chronograph Purple Dial');
    console.log('========================================================================');

    const file1 = 'Royal Oak Chronograph Selfwinding Chronograph Purple Dial_1.png'; // MAIN BANNER
    const file2 = 'Royal Oak Chronograph Selfwinding Chronograph Purple Dial_2.png';
    const file3 = 'Royal Oak Chronograph Selfwinding Chronograph Purple Dial_3.png';
    const file4 = 'Royal Oak Chronograph Selfwinding Chronograph Purple Dial_4.png';
    ...
    const mainBannerUrl = await convertToWebpAndUpload(path1, 'ro_chrono_selfwinding_purple_dial_main_1');
    const img2Url = await convertToWebpAndUpload(path2, 'ro_chrono_selfwinding_purple_dial_2');
    const img3Url = await convertToWebpAndUpload(path3, 'ro_chrono_selfwinding_purple_dial_3');
    const img4Url = await convertToWebpAndUpload(path4, 'ro_chrono_selfwinding_purple_dial_4');
    ...
    const existingProduct = await Product.findOne({
      name: 'Royal Oak Chronograph Selfwinding Chronograph Purple Dial'
    });

    let targetId = existingProduct?.id;
    if (!targetId) {
      const maxProduct = await Product.findOne().sort({ id: -1 });
      targetId = maxProduct && typeof maxProduct.id === 'number' ? maxProduct.id + 1 : 118;
    }
    ...
    const productPayload = {
      ...
      audience: 'Mens',
      factory: 'Selfwinding Chronograph Purple Dial',
      model: 'Royal Oak',
      reference: '26240BC.00.1320BC.01',
      material: '916L Stainless Steel case and bracelet, purple dial',
      size: '41 mm',
      ...
      description: 'The Audemars Piguet Royal Oak Chronograph Selfwinding in stainless steel features an exclusive, radiant purple "Grande Tapisserie" dial, matching subdials, and integrated steel bracelet.',
      features: [
        'Movement: Automatic chronograph movement',
        ...
        'Material Composition: 916L Stainless Steel case and bracelet, purple dial',
        'Diameter Size: 41 mm',
      ...
      isVisible: true,
      // Arabic translations
      nameAr: 'رويال أوك كرونوغراف أوتوماتيكية ميناء بنفسجي',
      brandAr: 'أوديمار بيغيه',
      modelAr: 'رويال أوك',
      materialAr: 'علبة وسوار من الفولاذ المقاوم للصدأ 916L، ميناء بنفسجي',
      ...
      descriptionAr: 'ساعة أوديمار بيغيه رويال أوك كرونوغراف أوتوماتيكية بميناء بنفسجي متألق بنمط "غراند تابسيري" الأيقوني وهيكل وسوار من الستانلس ستيل 916L عالي الجودة.',
      featuresAr: [
        ...
        'تركيبة المواد: علبة وسوار ستانلس ستيل 916L وميناء بنفسجي',
        'الأبعاد: 41 مم',
